...
//
#include "Spirver_ext.h"
#include <glslang/SPIRV/SpvTools.h>
#include <glslang/SPIRV/GlslangToSpv.h>


const char* Spirver_ext::stageNames[] = { "Vertex", "Fragment", "Geometry" };
const int Spirver_ext::stageCount = 3;
...
EShLanguage Spirver_ext::StageToGlslang(Stage stage)
{
	switch (stage)
	{
...
int Spirver_ext::StageToInt(Stage stage)
{
	switch (stage)
	{
...
Spirver_ext::Stage Spirver_ext::StageToSpirver(GLenum stage)
{
	switch (stage)
	{
...
Spirver_ext::Stage Spirver_ext::StageToSpirver(EShLanguage stage)
{
	switch (stage)
	{
...
Spirver_ext::Stage Spirver_ext::StageToSpirver(df::detail::SHADER_TYPE stage)
{
	switch (stage)
	{
	...
	default: return Spirver_ext::Stage::Vertex;
	}
}

Spirver_ext::Stage Spirver_ext::StageToSpirver(int i)
{
	switch (i)
	{
	case 0: return Stage::Vertex;
	case 1: return Stage::Fragment;
	case 2: return Spirver_ext::Stage::Geometry;
...
Spirver_ext::Stage Spirver_ext::StageToSpirver(const std::string& s)
{
...
std::string Spirver_ext::GetErrors()
{
...
bool Spirver_ext::printLog(GLuint object, LogType logType)
{
...
bool Spirver_ext::printLog(glslang::TShader* object)
{
...
bool Spirver_ext::printLog(glslang::TProgram* object)
{
	std::string log(object->getInfoLog());
	if (log.size() > 0)
	{
...
bool Spirver_ext::printLog(spv::SpvBuildLogger& object)
{
...
void Spirver_ext::printSpirvOptLog(spv_message_level_t level, const char* source,
...
void Spirver_ext::Init()
{
	glslang::InitializeProcess();
}

void Spirver_ext::Clean()
{
...
bool Spirver_ext::glslToAstShader(const std::string& source, glslang::TShader* shader, int uniformBase)
{
...
bool Spirver_ext::astShaderToAstProgram(glslang::TShader* shader, glslang::TProgram* program)
{
...
bool Spirver_ext::astProgramToSpirv(glslang::TProgram* program, std::vector<GLuint>& spirv, Stage stage)
{
	spv::SpvBuildLogger logger;
	glslang::SpvOptions spvOptions;
	spvOptions.disableOptimizer = true;
...
const TBuiltInResource Spirver_ext::DefaultTBuiltInResource = {
	/* .MaxLights = */ 32,
	/* .MaxClipPlanes = */ 6,
	/* .MaxTextureUnits = */ 32,
	/* .MaxTextureCoords = */ 32,
